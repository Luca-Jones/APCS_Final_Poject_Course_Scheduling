...
for course in courselist:
    ...
    # look through all students
    for i in range(len(student)):
        for j in range (len(student[i]._alternates)):
            for v in range(len(alpha) - 1):
                if (course._name == student[i]._alternates[j] and alpha[v] not in student[i].student_classes):
                    ...
                    if j > 3:
                        for seq in course.sequencing:
                            if student[i]._alternates.count(seq) > 0:
                                continue
                    ...
                    for pre in courselist:
                        ...
                        if pre.sequencing.count(course._name) > 0:
                            ...
                            # does the student have the prerequisite
                            if student[i]._alternates.count(pre._name) > 0:
